...
Arvados <- R6::R6Class(
    ...
    public = list(
        ...
        #' @param clusterID Create object on a remote federated cluster instead of the current one.
        #' @examples
        #' arv$collections_create(name = "collectionTitle", description = "collectionDescription", ownerUUID = "collectionOwner", properties = list("ROX37196928443768648" = "ROX37742976443830153"))
                                      ...
                                      ensureUniqueName = "false", clusterID = NULL)
        {
            ...
            if(!is.null(resource$errors)){
                if(identical(sub('Entity:.*',"", resource$errors), "//railsapi.internal/arvados/v1/collections: 422 Unprocessable ")){
                    cat(format("A collection with the given name already exists in this projects. If you want to update it use collections_update() instead"))
                }else{
                    stop(resource$errors)
                }
            }
        ...
        #' @param uuid The UUID of the Collection in question.
        #' @examples
        #' collection <- arv$collections_create(name = "newCollectionTitle", description = "newCollectionDescription", ownerUUID = "collectionOwner", properties = list("ROX37196928443768648" = "ROX37742976443830153"))
